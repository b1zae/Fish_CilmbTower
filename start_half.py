...
class WindowHandler:
    def __init__(self):
        ...
        # 初始化时不强制查找，避免启动就崩，改为在截图时查找
        pass

    def find_window(self):
        # 如果已经有了句柄且窗口有效，直接返回，不再重复遍历（提高性能）
        if self.window and win32gui.IsWindow(self.window):
            return

        def callback(hwnd, extra):
            try:
                # 只有窗口可见时才匹配，避免匹配到后台隐藏进程
        ...
        try:
            # 这里的 try 保护 EnumWindows 本身
            win32gui.EnumWindows(callback, None)
        except Exception as e:
            print(f"遍历窗口时忽略错误: {e}")

        # 移除原代码第47行那句多余的 win32gui.EnumWindows(callback, None)

        if not self.window:
            # 找不到也不要抛出异常让程序崩溃，而是打印日志
            # 这样主线程还可以继续运行，等待下一次尝试
            print(f"暂未找到【{self.window_title}】窗口，请确保游戏已打开...")
            self.window = None
        ...
        try:
            ...
            try:
                ...
                if win32gui.IsWindow(self.window):
                    ...
                    # 这一行 SetForegroundWindow 在Win10/11经常被拒绝，加上 try
                    try:
...
class WinOperator:
    ...
    def click(self, x, y):
        ...
        """
        try:
            # 每次点击时，从 handler 中获取最新的 window 句柄
            hwnd = self.handler.window
            ...
            if hwnd and win32gui.IsWindow(hwnd):
                ...
                # 尝试重新寻找窗口
                self.handler.find_window()
                if self.handler.window:
                    return self.click(x + left, y + top)  # 递归重试一次（注意坐标还原）
