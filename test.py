...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("咸鱼助手 v2.2 (智能容错版)")
        ...
        layout.addWidget(QLabel("新功能：长时间无响应时会自动点击屏幕中心"))
...
class MainWorker:
    def __init__(self, save_images=False, show_marker=False):
        ...
        # [新功能] 空闲计数器
        self.idle_counter = 0
        self.IDLE_CLICK_THRESHOLD = 10  # 约4-5秒
        ...
        while self.is_running:
            try:
                ...
                if not processed:
                    ...
                    if area:
                        ...
                        if res:
                            ...
                            stuck_keywords = ["挑战", "进攻", "闯关", "布阵", "下一塔"]
                            res = ocr.find_text_center(img, stuck_keywords)
                            if res:
                                rel_x, rel_y, txt = res
                                print(f"发现 [{txt}] -> 点击")
                                self.save_detection_image(img, txt)
                                ...
                                # 防卡死逻辑
                                if self.last_action_text == txt:
                                    self.stuck_counter += 1
                                else:
                                    self.stuck_counter = 1
                                print(f"  (连续点击'{txt}' {self.stuck_counter}/{self.STUCK_THRESHOLD} 次)")
                                self.last_action_text = txt

                                if self.stuck_counter >= self.STUCK_THRESHOLD:
                                    print(f"\n错误：连续点击'{txt}'无响应，自动停止。")
                                    self.stop()
                                    break

                                time.sleep(1.5)
                            else:
                                # 如果有其他按钮，也算作有活动，重置卡死计数
                                if ocr.find_text_center(img, ["确定"]):
                                    self.stuck_counter = 0
                ...
                if processed:
                    ...
                    # 如果没有任何操作，增加空闲计数
                    self.idle_counter += 1
                    time.sleep(0.2)  # 空闲时等待0.2秒

                    if self.idle_counter >= self.IDLE_CLICK_THRESHOLD:
                        print("\n长时间无操作，尝试点击屏幕中心以继续...")
                        rect = get_window_rect()
                        if rect:
                            center_x = (rect[0] + rect[2]) // 2
                            center_y = (rect[1] + rect[3]) // 2 + 250
                            operator.click(center_x, center_y)
                        self.idle_counter = 0  # 点击后重置计数
